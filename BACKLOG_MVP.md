...
- [ ] `E0-T5` Частично: добавлены GitHub Actions для CI (`backend/frontend checks`) и frontend autodeploy на VPS; тесты пока не добавлены.
...
- [x] `E4-T4` Добавлена автоматическая запись переходов в `task_status_history` (создание задачи, выбор исполнителя, отправка на проверку, отмена задачи).
